...
The LIFF on-page experience now auto-translates copy on the fly using a local dictionary-backed translator (no remote API calls). English remains the source language; other languages are rendered at runtime and cached in-memory to avoid repeat work.
...
## Notes about the local translator
- Translations are bundled with the app in `src/liff-web/src/lib/localTranslator.js` to avoid any external network calls.
- When a phrase is missing from the dictionary, the UI gracefully falls back to English so the app never blocks rendering.
